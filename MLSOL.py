...
class MLSOL():
    ...
    def _calculate_weight(self, X, y): 
        ...
        for i in range(numInstances):
            ...
            for j in range(numLabels):
                numMaj = 0
                if yData[self._labelIndices[j]] == self._minLabels:
                    for k in range(self._numOfNeighbors):
                        if yData[self._labelIndices[j]] != yData[result[k]][self._labelIndices[j]]:
                            numMaj+=1
        ...
        for i in range(numInstances):
            ...
            for j in range(numLabels):
                if scores[i][j] != None:
                    self._weights += scores[i][j]
            sumW+=self._weights[i]
